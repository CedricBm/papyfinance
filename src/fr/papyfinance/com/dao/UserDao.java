...
public class UserDao {
	...
	public User getByEmail(String email) {
		Session session = sessionFactory.openSession();
		session.beginTransaction();
		User o = (User) session.createQuery("from User where email = :semail").setParameter("semail", email)
				.uniqueResult();
		session.getTransaction().commit();
		session.close();
		return o;
	...
	public User getByLogin(String login) {
		Session session = sessionFactory.openSession();
		session.beginTransaction();
		User o = (User) session.createQuery("from User where login = :slogin").setParameter("slogin", login)
				.uniqueResult();
		session.getTransaction().commit();
		session.close();
		return o;
	...
	public ArrayList<User> getAllCompanyMember() {
		Session session = sessionFactory.openSession();
		session.beginTransaction();
		ArrayList<User> o = (ArrayList<User>) session.createQuery("from User where role.name = 'Membre société'")
				.list();
		session.getTransaction().commit();
		session.close();
		return o;
