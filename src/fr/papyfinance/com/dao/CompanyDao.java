...
public class CompanyDao {
	...
	public Company getByName(String name) {
		Session session = sessionFactory.openSession();
		session.beginTransaction();
		Company c = (Company) session
				.createQuery("from Company where name = :cname")
	...
	public Company getById(int id) {
		...
		session.beginTransaction();
		Company c = (Company) session
				.createQuery("from Company where id = :cid")
	...
	public ArrayList<Company> getAllCompanyNotAccredit() {
		Session session = sessionFactory.openSession();
		session.beginTransaction();
		ArrayList<Company> c = (ArrayList<Company>) session.createQuery(
				"from Company where is_confirmed = false").list();
		session.getTransaction().commit();
		session.close();
		return c;
	}
	...
	public ArrayList<Company> getAllWithAttribute(String attribute) {
		Session session = sessionFactory.openSession();
		session.beginTransaction();
		String query = "from Company where name like '%" + attribute
				+ "%'  OR website like '%" + attribute + "%' ";
		ArrayList<Company> c = (ArrayList<Company>) session.createQuery(query)
				.list();
		session.getTransaction().commit();
		session.close();
		return c;
